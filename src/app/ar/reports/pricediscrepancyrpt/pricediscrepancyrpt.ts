...
@Component({
    selector: 'pricediscrepancyrpt',
    templateUrl: './pricediscrepancyrpt.html',
    // providers: [DataEntryService] // New instance gets created
})
export class pricediscrepancyrpt implements OnDestroy, AfterViewInit {
    ...
    print() {
        ...
        var mParms = "pAnd=" + (this.fcid !== ''? " AND customeritems.fcid=" + this.fcid : " ");

        this.CompanySvc.ofCreateJasperReport ('customerbelowprice.pdf', mParms).subscribe((pResponse) => {
            // Open PDF file
            setTimeout(() => {
